...
wsModel::wsModel(const char* filepath) {
    ...
    animations = wsNew(wsHashMap<wsAnimation*>, wsHashMap<wsAnimation*>(wsNextPrime(11)));
    looping = false;
    timeScale = 1.0f;
    animPaused = false;
}

wsModel::wsModel(const char* myName, wsMesh* myMesh, const u32 myMaxAnimations = 11) {
    assetType = WS_ASSET_TYPE_MODEL;
    mesh = myMesh;
    name = myName;
    defaultAnimation = 0;
...
void wsModel::applyAnimation() {
    ...
    for (u32 v = 0; v < mesh->getNumVerts(); ++v) {
        posSum.set(0, 0, 0, 1);
        for (u32 w = 0; w < 1; ++w) {
            pos = verts[v].originalPos;
            pos -= mesh->getBaseSkel()[verts[v].weights[w].jointIndex].start;
